...
public abstract class Command {
	...
	/**
	 * exception catched in fileUpload in debug mode should be thrown to
	 * servlet response.
	 */
	protected Exception exception;

	/**
	 * connector configurarion.
	 */
	...
	/**
	 * runs commad. Initialize, sets response and execute command.
	 * @param request request
	 * @param response response
	 * @param configuration1 connector configuration
	 * @param params additional execute params.
	 * @throws ConnectorException when error occure
	 */
	...
	protected boolean checkParam(final String reqParam)
		...
		if (Pattern.compile(Constants.INVALID_PATH_REGEX).matcher(reqParam)
				.find()) {
			throw new ConnectorException(
	...
	/**
	 * gets current folder request param
	 * or sets default value if it's not set.
	 * @param request request
	 */
	protected void getCurrentFolderParam(final HttpServletRequest request) {
		...
		if (currFolder == null || currFolder.equals("")) {
			...
			this.currentFolder = PathUtils.addSlashToBeginning(PathUtils
					.addSlashToEnd(currFolder));
		}
	}
}
