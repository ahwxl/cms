...
public class FreeMarkHibernateDao extends HibernateDao {
    ...
    public List getFmCatalogList(FmCatalog vo) {
        ...
        List list = this
            .getSession()
            .createQuery(
                " from FmCatalog a where a.pCatalogId = :parentCatalogID order by a.orderId ")
    ...
    public List getParentFmCatalogList(FmCatalog vo) {
        ...
        List list = this
            .getSession()
            .createQuery(
                " from FmCatalog a where a.pCatalogId = (select a.pCatalogId from FmCatalog a where a.catalogId = :catalogId ) ")
    ...
    public FmProduct queryProductById(FmProduct vo) {
        return (FmProduct) this.findUnique(" From FmProduct a where a.productId = ? ",
            vo.getProductId());
    }
    ...
    public List getFmProductListByCatalogId(FmProduct vo) {
        List list = this.getSession()
            .createQuery("from FmProduct a where a.catalogId = :catalogId")
            .setString("catalogId", vo.getCatalogId()).list();
    ...
    public List getAllRoles(SysRole vo) {
        ...
        if (StringUtils.isNotEmpty(vo.getRoleName())) {
            list = this.getSession().createQuery("from SysRole a where a.roleName like :roleName")
                .setString("roleName", "%" + vo.getRoleName() + "%").list();
        } else {
    ...
    public SysRole getRoleById(SysRole vo) {
        ...
        SysRole sysRole = (SysRole) this.getSession()
            .createQuery("from SysRole a where a.roleId = :roleId")
    ...
    public List getAllUsers(SysUser vo) {
        ...
        List list = this.getSession()
            .createQuery("select new SysUser( a.userName,a.enabled) from SysUser a").list();
    ...
    public IPagination getDepartmentListPage(SysDepartment po) {
        String hql = "select new SysDepartment( a.departid,a.departname,a.departdesc,a.pdepartid,a.sortid) from SysDepartment a where a.pdepartid ='"
                     + po.getPdepartid() + "'";
