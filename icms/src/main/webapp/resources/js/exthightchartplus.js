...
* @requires HighChart 2.0.5 {@link http://www.highcharts.com} Todo:
*           setSerieStyle, setStyle, Multiple datastores (0.5.0)
* 
*/
/**
* @class Ext.ux.HighChart
* @extends Ext.BoxComponent The Ext.chart package provides the capability to
*          visualize data with javascript based charting. Each chart binds
*          directly to an Ext.data.Store enabling automatic updates of the
*          chart. To change the look and feel of a chart, see the
*          {@link http://www.highcharts.com/ref/#chart} config options.
* @constructor
* @xtype highchart
*/
Ext.ux.HighChart = Ext.extend(Ext.BoxComponent, {
        ...
        /**
         * @cfg {Object} defaultSerieType Sets styles for this chart. This contains
         *      default styling, so modifying this property will <b>override</b>
        ...
        /**
         * @cfg {Boolean} resizable True to allow resizing, false to disable
         *      resizing (defaults to true).
         */
        resizable : true,

        /**
         * @cfg {Integer} updateDelay (defaults to 0)
         */
        updateDelay : 0,

        /**
         * @cfg {Object} loadMask An {@link Ext.LoadMask} config or true to mask the
         *      chart while loading. Defaults to false.
         */
        loadMask : false,
         ...
         * @param {Boolean}
         *            append the serie. Defaults to true
         */
        addSeries : function(series, append) {
                append = (append == null) ? true : false;
                var n = new Array(), c = new Array(), cls, serieObject;
                // Add empty data to the serie or just leave it normal. Bug in
                // HighCharts?
                for (var i = 0; i < series.length; i++) {
                        var serie = series;
                        if (!serie.serieCls) {
                                if (serie.type != null || this.defaultSerieType != null) {
                                        cls = Ext.ux.HighChart.Series.get(serie.type != null
                                                        ? serie.type
                                                        : this.defaultSerieType)
                                } else {
                ...
                if (this.chart) {
                        if (!append) {
                                ...
                                this.chartConfig.series = this.chartConfig.series
                                                ? this.chartConfig.series.concat(c)
                                                : c;
                                this.series = this.series ? this.series.concat(n) : n;
                        }
                        for (var i = 0; i < c.length; i++) {
                                this.chart.addSeries(c, true);
                        }
                        this.refresh();
         ...
         * 
         * @param {String}
         *            title Text to set the subtitle
         */
                ...
                if (this.chart && this.rendered) {
                        if (this.resizable) {
                                for (var i = 0; i < this.series.length; i++) {
                                        this.series.visible = this.chart.series.visible;
                                }
        ...
        // xAxis draw number determine step
        xAxisDrawNumber : 12,
        // private
        updatexAxisData : function() {
                ...
                if (this.xField && this.store) {
                        ...
                        for (var j = 0; j < dataCount; j++) {
                                data.push(items[j].data[this.xField]);
                        }

                        var step = 1;
                        if (dataCount >= this.xAxisDrawNumber) {
                                step = Math.round(dataCount / this.xAxisDrawNumber);
                        }
                        if (this.chart) {
                                this.chart.xAxis[0].setOptions({
                                        labels : {
                                                step : step
                                        }
                                });
                                ...
                                this.chartConfig.xAxis[0].tickInterval = step;
                                this.chartConfig.xAxis[0].labels.step = step;
        ...
        bindComponent : function(bind) {
                /**
                 * Make the chart update the positions positions are based on the window
                 * object and not on the owner object.
                 */
         ...
         * 
         * @param {Store}
         *            store The store to bind to this chart
         */
        ...
        refresh : function() {
                if (this.store && this.chart) {
                        ...
                        // Update the series
                        for (i = 0; i < seriesCount; i++) {
                                if (this.series.useTotals) {
                                        this.chart.series.setData(this.series.getTotals())
                                } else if (data.length > 0)
                                        this.chart.series.setData(data,
                                                        (i == (seriesCount - 1))); // true == redraw.
                        }
        ...
        refreshRow : function(record) {
                ...
                if (this.chart) {
                        for (var i = 0; i < this.chart.series.length; i++) {
                                ...
                                var point = this.series.getData(record, index);
                                if (this.series.type == 'pie' && this.series.useTotals) {
                                        this.series.update(record);
                                        this.chart.series.setData(this.series.getTotals());
                                } else
        ...
        onRemove : function(ds, record, index, isUpdate) {
                for (var i = 0; i < this.series.length; i++) {
                        ...
                        if (s.type == 'pie' && s.useTotals) {
                                ...
                                this.chart.series.setData(s.getTotals())
                        } else {
                                this.chart.series.data[index].remove(true)
                        }
                }
...
/**
* @class Ext.ux.HighChart.Series This class registers all available series, and
...
/**
* @class Ext.ux.HighChart.Serie Series class for the highcharts widget.
* @constructor
*/
...
* @extends Ext.ux.HighChart.Serie SplineSerie class for the charts widget.
* @constructor
*/
...
* @extends Ext.ux.HighChart.Serie ColumnSerie class for the charts widget.
* @constructor
*/
...
* @extends Ext.ux.HighChart.Serie BarSerie class for the charts widget.
* @constructor
*/
...
/**
* @class Ext.ux.HighChart.SplineSerie
* @extends Ext.ux.HighChart.Serie LineSerie class for the charts widget.
* @constructor
*/
...
/**
* @class Ext.ux.HighChart.SplineSerie
* @extends Ext.ux.HighChart.Serie AreaSerie class for the charts widget.
* @constructor
*/
...
/**
* @class Ext.ux.HighChart.SplineSerie
* @extends Ext.ux.HighChart.Serie AreasplineSerie class for the charts widget.
* @constructor
*/
...
* @extends Ext.ux.HighChart.Serie ScatterSerie class for the charts widget.
* @constructor
*/
...
* @extends Ext.ux.HighChart.Serie PieSerie class for the charts widget.
* @constructor
*/
        ...
        constructor : function(config) {
                ...
                if (this.useTotals) {
                        ...
                        for (var i = 0; i < length; i++) {
                                this.columnData[this.columns] = 100 / length;
                        }
                }
        },
